"""Tests for CloudArchitectAgent."""
from __future__ import annotations
import json
import pytest
from unittest.mock import AsyncMock, MagicMock
from archon.agents.cloud_architect import CloudArchitectAgent
...
def _mock_response():
    return json.dumps({
        "findings": [{
            "id": "CA-001", "title": "Test finding",
            "description": "Test desc", "severity": "MEDIUM",
            "domain": "cloud-architect", "file_path": None,
...
class TestCloudArchitectAgent:
    def test_domain(self, mock_llm, mock_searcher):
        store = MagicMock()
        store.query = AsyncMock(return_value=[])
        agent = CloudArchitectAgent(mock_llm, [mock_searcher], RAGRetriever(store))
        assert agent.domain == "cloud-architect"
    ...
    async def test_run_returns_output(self, mock_llm, mock_searcher):
        ...
        assert output.domain == "cloud-architect"
