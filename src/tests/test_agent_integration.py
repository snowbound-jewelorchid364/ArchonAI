"""Tests for IntegrationArchitectAgent."""
from __future__ import annotations
import json
import pytest
from unittest.mock import AsyncMock, MagicMock
from archon.agents.integration_architect import IntegrationArchitectAgent
...
def _mock_response():
    return json.dumps({
        "findings": [{
            "id": "IA-001", "title": "Test finding",
            "description": "Test desc", "severity": "MEDIUM",
            "domain": "integration-architect", "file_path": None,
...
class TestIntegrationArchitectAgent:
    def test_domain(self, mock_llm, mock_searcher):
        store = MagicMock()
        store.query = AsyncMock(return_value=[])
        agent = IntegrationArchitectAgent(mock_llm, [mock_searcher], RAGRetriever(store))
        assert agent.domain == "integration-architect"
    ...
    async def test_run_returns_output(self, mock_llm, mock_searcher):
        ...
        assert output.domain == "integration-architect"
